<div id="jazzcash-payment-container" style="margin-top: 15px;">
    ...
    <div class="form-group">
        ...
        <input type="text" id="jazzcash_phone" name="jazzcash_phone" class="form-control" placeholder="{{ __('Enter your Jazzcash registered phone') }}" value="{{ old('jazzcash_phone') }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        function toggleJazzcashFields() {
            const selected = document.querySelector('input[name="payment_gateway"]:checked').value;
            const container = document.getElementById('payment-jazzcash');
        ...
        toggleJazzcashFields();

        document.querySelectorAll('input[name="payment_gateway"]').forEach(radio => {
            radio.addEventListener('change', toggleJazzcashFields);
        });
    });
</script>
